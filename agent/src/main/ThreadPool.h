...
public:
    ...
    {
        idleThrNum = size < 1 ? 1 : size;
        for (size = 0; size < idleThrNum; ++size)
        {   //初始化线程数量
            pool.emplace_back(
                [this]
                { // 工作线程函数
...
public:
    ...
    auto AddTaskToQueue(F&& f, Args&&... args) ->std::future<decltype(f(args...))>
    {
    ...
    bool IsFullQueue()
    {
        ...
        return tasks.size() < MAX_QUEUE_NUM ? false : true; 
    }
